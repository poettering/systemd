...
/* TODO:
 *
 *   → try to use bind mounts if overlayfs not necessary
 *   → add service file to run this at boot
 */
...
static int extend_overlayfs_lower(char **lower, const char *path) {
        ...
        if (!strextend_with_separator(lower, ":", escaped))
                return log_oom();
...
static int merge_hierarchy(
        ...
        _cleanup_free_ char *lower = NULL, *options = NULL, *resolved_hierarchy = NULL, *f = NULL, *buf = NULL;
        ...
        r = extend_overlayfs_lower(&lower, meta_path);
        if (r < 0)
                return r;
        ...
        STRV_FOREACH(p, paths) {
                ...
                r = extend_overlayfs_lower(&lower, resolved);
                if (r < 0)
                        return r;
        ...
        if (!populated) /* No extension with files in this hierarchy? Then don't do anything. */
                return 0;

        if (resolved_hierarchy) {
                /* Add the host hierarchy as last (lowest) layer in the stack */
                r = extend_overlayfs_lower(&lower, resolved_hierarchy);
                if (r < 0)
                        return r;
        }

        options = strjoin("lowerdir=", lower);
        if (!options)
                return log_oom();
        ...
        /* Now mount the actualy overlayfs */
        r = mount_nofollow_verbose(LOG_ERR, "sysext", overlay_path, "overlay", MS_RDONLY, options);
        if (r < 0)
                return r;

        /* The superblock is read-only. Let's also mark the bind mount read-only. Extra turbo safety 😎 */
...
static int help(void) {
        ...
        printf("%s [OPTIONS...] [DEVICE]\n"
               "\n%sMerge extension images into /usr/ and /opt/ hierarchies.%s\n\n"
