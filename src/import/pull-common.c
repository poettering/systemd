...
int pull_make_verification_jobs(
        ...
        /* Acquire the checksum file if verification or sinature verification is requested and the main file
...
int pull_verify(ImportVerify verify,
        ...
        _cleanup_free_ char *url = NULL, *fn = NULL;
...
int pull_url_needs_checksum(const char *url) {
         ...
         * files — except for gpg signature files and SHA256SUMS files and the like, which are validated a
         * validation tool like gpg. */
