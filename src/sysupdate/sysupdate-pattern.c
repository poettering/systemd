...
static PatternElement *pattern_element_free_all(PatternElement *e) {
        ...
        return pattern_element_free_all(n);
}
...
int pattern_format(
        ...
        LIST_FOREACH(elements, e, elements) {
                ...
                case PATTERN_PARTITION_FLAGS: {
                        char formatted[sizeof(uint64_t) * 2 + 1];

                        if (!fields->partition_flags_set)
                                return -ENXIO;

                        xsprintf(formatted, "%" PRIx64, fields->partition_flags);

                        if (!strextend(&j, formatted))
                                return -ENOMEM;
                ...
                case PATTERN_MTIME: {
                        char formatted[DECIMAL_STR_MAX(usec_t) + 1];

                        if (fields->mtime == USEC_INFINITY)
                                return -ENXIO;

                        xsprintf(formatted, "%" PRIu64, fields->mtime);
                        if (!strextend(&j, formatted))
                                return -ENOMEM;
                ...
                case PATTERN_MODE: {
                        ...
                        xsprintf(formatted, "%o", fields->mode);
                        if (!strextend(&j, formatted))
                                return -ENOMEM;
                ...
                case PATTERN_SIZE: {
                        char formatted[DECIMAL_STR_MAX(uint64_t)+1];

                        if (fields->size == UINT64_MAX)
                                return -ENXIO;

                        xsprintf(formatted, "%" PRIu64, fields->size);
                        if (!strextend(&j, formatted))
                                return -ENOMEM;
                        break;
                }
                ...
                case PATTERN_TRIES_DONE: {
                        ...
                        xsprintf(formatted, "%" PRIu64, fields->tries_done);
                        if (!strextend(&j, formatted))
                                return -ENOMEM;
                        break;
                }
                ...
                case PATTERN_TRIES_LEFT: {
                        ...
                        xsprintf(formatted, "%" PRIu64, fields->tries_left);
                        if (!strextend(&j, formatted))
                                return -ENOMEM;
                        break;
                }
