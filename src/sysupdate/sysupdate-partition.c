...
static int fdisk_partition_set_attrs_as_uint64(
        ...
        for (unsigned i = 0; i < sizeof(flags) * 8; i++) {
                char formatted[DECIMAL_STR_MAX(unsigned)];

                if (!FLAGS_SET(flags, UINT64_C(1) << i))
                        continue;

                xsprintf(formatted, "%u", i);
                if (!strextend(&attrs, ",", formatted))
                        return -ENOMEM;
        }
...
int read_partition_info(
        ...
        *ret = (PartitionInfo) {
                ...
                .read_only = flags & gpt_partition_type_uuid_to_supported_flags(ptid) & GPT_FLAG_READ_ONLY,
        };
...
int patch_partition(const char *device, const PartitionInfo *info, PartitionChange change) {
        ...
        if (change & PARTITION_UUID) {
                ...
                r = fdisk_partition_set_uuid(pa, id128_to_uuid_string(info->uuid, ids));
                if (r < 0)
        ...
        tweak_ro =
                ...
                FLAGS_SET(gpt_partition_type_uuid_to_supported_flags(info->type), GPT_FLAG_READ_ONLY);
        ...
        if (change & PARTITION_FLAGS) {
                ...
                if (r < 0)
                        return log_error_errno(r, "Failed update partition flags: %m");
        ...
        if (r < 0)
                return log_error_errno(r, "Failed update partition: %m");
