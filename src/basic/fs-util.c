...
int fs_make_read_only(int fd) {
         ...
         * 3. If the fd refers to a regular file we'll drop the x bits.
...
static int unlinkat_harder(int dirfd, const char *pathname) {
...
int install_file(int source_atfd, const char *source_name,
         ...
         * 2. Optionally marks the file/directory read-only using fs_make_read_only()
                ...
                if (flags & INSTALL_REPLACE) {
                        ...
                        if (renameat(source_atfd, source_name, target_atfd, target_name) < 0) {
                                ...
                                if (renameat2(source_atfd, source_name, target_atfd, target_name, RENAME_EXCHANGE) < 0) {
                                        ...
                                        r = unlinkat_harder(target_atfd, target_name);
                                        if (r < 0)
                                        ...
                                        r = unlinkat_harder(source_atfd, source_name);
                                        if (r < 0)
